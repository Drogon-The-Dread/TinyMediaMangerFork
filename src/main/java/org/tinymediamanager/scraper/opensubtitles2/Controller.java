...
import org.tinymediamanager.scraper.opensubtitles2.service.Opensubtitles2Service;
    ...
    if (restAdapter == null) {
      ...
      builder.client(TmmHttpClient.newBuilder().addInterceptor(chain -> {
        ...
        if (StringUtils.isNotBlank(token)) {
          request.addHeader(PARAM_AUTHORIZATION, token);
        }
        request.header("User-Agent", "tinyMediaManager");
